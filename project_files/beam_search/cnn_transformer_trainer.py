...
class BrainToText_Trainer:
    def __init__(self, args):
        ...
        self.logger.info(f"Initialized Transformer model with Beam Search support")
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if i % self.args['batches_per_val_step'] == 0 or i == ((self.args['num_training_batches'] - 1)):
                ...
                self.logger.info(f'Val batch {i}: ' +
                                 f'PER (greedy): {val_metrics["avg_PER_greedy"]:.4f} ' +
                                 f'PER (beam): {val_metrics["avg_PER_beam"]:.4f} ' +
                ...
                if self.args['log_individual_day_val_PER']:
                    for day in val_metrics['day_PERs'].keys():
                        self.logger.info(
                            f"{self.args['dataset']['sessions'][day]} val PER (greedy): {val_metrics['day_PERs'][day]['total_edit_distance_greedy'] / val_metrics['day_PERs'][day]['total_seq_length']:0.4f} | "
                            f"PER (beam): {val_metrics['day_PERs'][day]['total_edit_distance_beam'] / val_metrics['day_PERs'][day]['total_seq_length']:0.4f}")

                # Save metrics - use beam search PER as primary metric
                val_PERs.append(val_metrics['avg_PER_beam'])
                val_losses.append(val_metrics['avg_loss'])
                val_results.append(val_metrics)

                # Determine if new best day. Based on if PER is lower (use beam search PER)
                new_best = False
                if val_metrics['avg_PER_beam'] < self.best_val_PER:
                    self.logger.info(f"New best test PER (beam) {self.best_val_PER:.4f} --> {val_metrics['avg_PER_beam']:.4f}")
                    self.best_val_PER = val_metrics['avg_PER_beam']
                    self.best_val_loss = val_metrics['avg_loss']
                    new_best = True
                elif val_metrics['avg_PER_beam'] == self.best_val_PER and (val_metrics['avg_loss'] < self.best_val_loss):
        ...
        self.logger.info(f'Best avg val PER (beam) achieved: {self.best_val_PER:.5f}')
    ...
    def validation(self, loader, return_logits=False, return_data=False):
        '''
        Calculate metrics on the validation dataset using both greedy and beam search decoding
        '''
        self.model.eval()
        ...
        metrics['decoded_seqs_greedy'] = []
        metrics['decoded_seqs_beam'] = []
        metrics['true_seq'] = []
        ...
        for d in range(len(self.args['dataset']['sessions'])):
            if self.args['dataset']['dataset_probability_val'][d] == 1:
                day_per[d] = {'total_edit_distance_greedy': 0, 'total_edit_distance_beam': 0, 'total_seq_length': 0}
        ...
        # Beam search parameters from config
        num_beams = self.args.get('beam_search', {}).get('num_beams', 5)
        length_penalty = self.args.get('beam_search', {}).get('length_penalty', 1.0)
        max_decode_length = self.args.get('beam_search', {}).get('max_decode_length', 100)
        ...
        for i, batch in enumerate(loader):
            ...
            with torch.no_grad():
                with torch.autocast(device_type="cuda", enabled=self.args['use_amp'], dtype=torch.bfloat16):
                    ...
                    # ====================================================
                    # ENCODE ONCE FOR BOTH DECODING METHODS
                    # ====================================================
                    ...
                    # ====================================================
                    # PASS 2: GREEDY DECODING
                    # ====================================================
                    generated_greedy = self.model.greedy_decode(memory, max_length=max_decode_length)

                    # ====================================================
                    # PASS 3: BEAM SEARCH DECODING
                    # ====================================================
                    generated_beam = self.model.beam_search_decode(
                        memory, 
                        num_beams=num_beams, 
                    ...
                    batch_edit_distance_greedy = 0
                    batch_edit_distance_beam = 0
                    decoded_seqs_greedy = []
                    decoded_seqs_beam = []

                    for iterIdx in range(batch_size):
                        # 1. Process Greedy Predicted Sequence
                        raw_pred_greedy = generated_greedy[iterIdx][1:]  # Remove SOS
                        valid_indices_greedy = []
                        for token in raw_pred_greedy:
                            if token == EOS_IDX: break
                            if token == PAD_IDX: continue
                            valid_indices_greedy.append(token.item())
                        
                        decoded_seq_tensor_greedy = torch.tensor(valid_indices_greedy, device=self.device, dtype=torch.long)
                        decoded_seqs_greedy.append(np.array(valid_indices_greedy))

                        # 2. Process Beam Search Predicted Sequence
                        raw_pred_beam = generated_beam[iterIdx][1:]  # Remove SOS
                        valid_indices_beam = []
                        for token in raw_pred_beam:
                            if token == EOS_IDX: break
                            if token == PAD_IDX: continue
                            valid_indices_beam.append(token.item())
                        
                        decoded_seq_tensor_beam = torch.tensor(valid_indices_beam, device=self.device, dtype=torch.long)
                        decoded_seqs_beam.append(np.array(valid_indices_beam))

                        # 3. Process True Sequence
                        raw_true = labels[iterIdx][0: phone_seq_lens[iterIdx]]

                        # 4. Calc Edit Distance for both
                        dist_greedy = F.edit_distance(decoded_seq_tensor_greedy, raw_true)
                        dist_beam = F.edit_distance(decoded_seq_tensor_beam, raw_true)
                        batch_edit_distance_greedy += dist_greedy
                        batch_edit_distance_beam += dist_beam

            # Update totals
            day = batch['day_indicies'][0].item()

            day_per[day]['total_edit_distance_greedy'] += batch_edit_distance_greedy
            day_per[day]['total_edit_distance_beam'] += batch_edit_distance_beam
            day_per[day]['total_seq_length'] += torch.sum(phone_seq_lens).item()

            total_edit_distance_greedy += batch_edit_distance_greedy
            total_edit_distance_beam += batch_edit_distance_beam
            total_seq_length += torch.sum(phone_seq_lens)

            # Record metrics
            if return_logits:
                metrics['logits'].append(tf_logits.cpu().float().numpy())
                gen_lens = torch.full((batch_size,), generated_beam.size(1), device='cpu')
                metrics['n_time_steps'].append(gen_lens.numpy())

            if return_data:
                metrics['input_features'].append(batch['input_features'].cpu().numpy())

            metrics['decoded_seqs_greedy'].append(decoded_seqs_greedy)
            metrics['decoded_seqs_beam'].append(decoded_seqs_beam)
        ...
        avg_PER_greedy = total_edit_distance_greedy / total_seq_length
        avg_PER_beam = total_edit_distance_beam / total_seq_length

        metrics['day_PERs'] = day_per
        metrics['avg_PER_greedy'] = avg_PER_greedy
        metrics['avg_PER_beam'] = avg_PER_beam
        metrics['avg_PER'] = avg_PER_beam  # For backward compatibility
